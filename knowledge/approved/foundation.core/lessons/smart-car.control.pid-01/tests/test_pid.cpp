...
int main()
{
    ...
    expect_near(integral.update(-2.0, 1.0), -0.5, "integral lower clamp");
    expect_near(integral.update(9.0, 0.0), 0.0, "invalid dt returns zero for integral");
    expect_near(integral.update(0.0, 1.0), -0.5, "invalid dt preserves integral");
    integral.reset();
    expect_near(integral.update(0.0, 1.0), 0.0, "reset clears integral");
    ...
    expect_near(derivative.update(-100.0, -1.0), 0.0, "negative dt returns zero");
    expect_near(derivative.update(6.0, 0.5), 2.0, "invalid dt preserves previous error");
