...
- `foundation.core 1.0.0`（198 题）已经完成首发审核，发布记录见仓库 README。第三方草稿不能继承这个审核状态，本机不运行其参考代码；执行验证按约定交给 GitHub Actions Docker。
- 0.8.0 新增 Exercism C/C++ 整体目录扫描与批量草稿导入，首批固定 32 个不同母题，操作及限制见 [批量获取指南](exercism-import.md)。
